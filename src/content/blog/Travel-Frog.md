...
![](https://ik.imagekit.io/Amyxue/Homepage_Amy/blog_o6XLf9Ljy)
...
这和后面年终总结的分享奖励一起提醒了我：一个奖励“数值上到账”并不代表行为已经复刻正确。奖励从哪里出现、是否经过邮箱、什么时候出现红点，本身也是游戏体验的一部分。
...
### Stage31-fix.旅行明信片概率调整


旅行结算继续从“能出发”推进到“带什么会带回什么”。食物、幸运物、博物馆票、工具和手工护符共同影响目的地、照片、特产、收藏品与旅行时长。

博物馆门票保持明确的定向效果；路标、星星灯、猫猫信纸、幸运花与其他幸运物则按照各自的地点偏置或照片池生效。节气食品、月饼、周年庆蛋糕和其他特殊食物也都连接到对应的照片组，不再只作为普通食物消耗。

旅行明信片恢复为通常随机带回；彩叶幸运草是例外，会优先补齐尚未获得的照片。这样既保留了原本旅行的随机性，也让彩草仍有明确的收集价值。

### Stage32-fix.嘟嘟商人、聚会与夕拾家具

嘟嘟不再常驻庭院。它会在中午到午夜的窗口内选择一段随机停留时间，到点离开；小摊商品也改为有限格位，而不是把全部物品同时堆进商店。

设计图、联动家具和幸运物都补进了各自的购买规则：
...
森之国度联动家具、古琴展博古家具、夕拾相关家具与常规种子、肥料一起进入嘟嘟可用商品池。旅友聚会返程也补回了三叶草、抽奖券、友情绘本新页和未获得夕拾家具等结果；已经拥有的图鉴页和家具不会重复作为新收集奖励出现。

### Stage33-fix.庭院、花圃与青蛙动作条件

花圃和堆肥不再只是数值面板。投入堆肥箱后的材料会先经历可取回阶段，随后锁定并开始发酵；锁定材料在花圃列表中显示为不可点击，完成吸收后才转化为土地肥力。不同肥力档位、花朵显示、播种、好友播种、收获和挂兜领取都使用同一份庭院状态。

家具工作台也重新与青蛙动作绑定：持有材料和工具并不意味着立刻制作，青蛙需要在家，等待随机开工时间，再经过一段随机制作时间才会完成家具。砍竹子、制作、用餐、阅读、睡眠和夏季吃西瓜等动作同样增加了时间、季节、餐桌食物或工作台任务等条件；青蛙外出时不会再在庭院错误地执行室内制作动作。

### Stage34.家具、手工品与限时称号

家具系统继续补齐特殊材料的产出和全部可制作款式。正丹纸等可对应多种主题的材料会在允许的正式主题中随机选择；彩带可以生成不同庆典家具；植物果实、手工合页和彩色编绳分别覆盖不倒翁、堆肥箱和挂兜的全部款式。

护符木片、木质护符、祈愿物和印章也保留材料获得、工具制作、制作动作与旅行效果的完整链路。称号方面，原活动期才能看到的称号改为离线版常驻内容：已满足条件会解锁、弹出提示并可更换；“蛙选之人”等限时称号有稳定的本地获得状态，不再随着活动结束而消失。
...
### Stage35.博物馆大冒险、中秋贺卡、春节贺卡、扭蛋机与周年庆蛋糕等限时活动

原本依赖活动服务的页面已经改成离线版可长期进入的内容：博物馆大冒险、中秋贺卡、春节贺卡、扭蛋机与周年庆蛋糕都各自保留活动本身的状态机、奖励和每日进度。

这些入口不再只靠客户端临时显示，而是统一写入本地存档：
...
博物馆大冒险的探索次数、每日登录和分享增加次数会按活动进度保存；扭蛋机则保留每日两项任务、待领取扭蛋币、快速完成、抽取记录和阶段奖励。蛋糕活动中的分享、制作、领取材料、分层制作、问答和点蜡烛继续沿用原有的顺序，而不是一次性跳到完成状态。

#### 节日贺卡的购买与刷新

中秋和春节贺卡都补回了可持续使用的材料循环。

中秋背景的“首次免费”现在是一份独立的购买历史，不再因为背景已经被放到贺卡上而重新变成免费；第一次购买后，后续背景购买固定消耗 100 三叶草。贴纸、背景、当前贺卡、已寄出记录和寄出后的奖励都使用同一份本地数据。

春节贺卡的小铺则按本地日期刷新每日福袋额度：
...
这样第二天不会停留在前一天的灰色售罄状态，分享、兑换和任务贴纸的数量也仍会在卡片制作与礼盒奖励中连续保存。

#### 分享任务与活动奖励同步

活动中的分享按钮现在会将实际分享动作回传到对应任务，而不是只关闭客户端弹窗。
...
扭蛋机的分享任务、每日任务列表和待领取扭蛋币使用同一份活动状态。完成任务后，扭蛋币会先进入待领取队列，打开扭蛋机即可自动收取并用于扭蛋；任务、扭蛋币和抽取记录都会随本地存档保存。

#### 活动入口与本地服务状态

这两天还逐个补齐了活动入口与服务端状态之间的连接。大冒险转盘、春节和中秋贺卡小铺、扭蛋机任务、周年蛋糕、许愿池和新手引导都不再依赖已经下线的活动服务。

每个按钮都会走到明确的本地状态转换，而不是只在客户端播放一次效果：
...
因此重新启动游戏后，旅行、照片、庭院、家具、称号和活动页面都会从同一份个人存档继续运行。

### Stage32.许愿池与空白存档的新手流程

许愿池补成了可以从零开始使用的离线入口。初始许愿币、可许愿的庭院物品、获得次数和领取结果都放在活动状态中；每个可获得物品保留自己的数量上限，获得后会直接进入家具或庭院库存。

全新空白存档的新手流程也补上了开门、步骤确认和完成奖励：
...
新手奖励只能领取一次，已有存档不会因为重新进入游戏而重复得到。


### Stage36-fix.过路照构图与旅行照片校准

普通照片不是一张固定背景。它由远景、道路或水面、近景、青蛙、随身工具和可能出现的旅伴组成；其中同一地点还会根据工具、同行对象和随机组选择不同构图。

这次继续按客户端图层、正式图片尺寸和攻略截图校准普通过路照。海边、道路、山地和城镇等场景重新按正确的前后层关系放置，不再把道路、沙滩和水面拆到错误的上下区域。青蛙与工具也会落在各自场景的正确活动区域，而不是统一从左下角开始估算。
...
静态场景优先使用已经确认的正式模板；普通随机场景保留多个可选位置与构图组，使同一条旅行路线不会每次都生成完全相同的照片。


# 2026.9.19

### Stage37.官服周末抓包与旅行、聚会回礼闭环

周末补齐了正式服的旅行返程、聚会返程与“周末的小插曲”抓包，并据此把离线服中原先只具备入口的回礼流程接回实际状态。旅行不再只结算固定的照片和特产：返程时还可以出现意外收获；照片的分享奖励也改成点击即可领取三叶草，不再跳转外部分享。

广告相关的任务入口保留在原位置，但离线版点击后直接完成奖励条件。这样活动页面仍保持原来的操作路径，同时不会依赖广告平台或第三方页面。
...
### Stage38.节日周期、春节宝箱与活动货架

限时活动重新分成“按自然日期出现”和“离线常驻”两类。中秋贺卡以每年中秋前后十天开放；春节贺卡以农历正月初一前后十五天开放；周年蛋糕固定在 12 月 14 日至次年 1 月 12 日。六一当天登录会获得棒棒糖，儿童节称号只解锁一次。

春节活动期间，嘟嘟每天会带来半价正丹纸；春节贺卡的金、红宝箱不再只按寄送次数决定，而是读取卡片背景和三张贴纸的组合。正式配置中的五组必出金箱搭配已经接入，其余组合仍会按卡片组合产生不同结果。
...
大冒险、扭蛋机和许愿池则作为离线版常驻内容保存。它们不再因原活动日期结束而关闭，面板和庭院入口也删除了“截止日期”“常驻”等提示，保持与普通长期功能一致的显示。

### Stage9-fix.庭院旅友的停留与回礼时机

旅友到访的间隔继续使用冷却时间加随机出现检查；到达庭院后，未喂食的旅友会随机停留 2 至 6 小时，而不是固定六小时。

喂食不会立即把回礼塞进邮箱。服务端会先记录旅友的口味、回礼或聚会邀请结果；旅友会再随机停留 2 至 4 小时，到时才离开并结算礼物、友情绘本或聚会邀请。花圃中的旅友播种、成长协助也在离开结算时一并完成。
...
普通离线版已经具备完整的日常循环。后续工作转入存档体验修正，以及面向收集、查看和测试的图鉴版。

# 2026.9.20

## 🐸图鉴版：独立端口、独立存档与全收集初始状态

图鉴版不是在普通存档上直接改数值，而是使用独立应用包、独立本地服务器端口和独立存档：
...
这样两套游戏可以同时安装、切换和测试，普通版的旅行进度、庭院状态和活动记录不会被图鉴版覆盖。

图鉴版的定位是“可查看、可试用、可继续互动的完整图鉴”，而不是把普通版的每一次获取流程都强行跳过。一次性收集内容在进入图鉴版时已解锁：旅行装备、家具、故事、称号、友情绘本、手工品、百科、地图、纪念品、特产、博物馆典藏和固定旅行照片都能直接查看。

### 1.图鉴版的无限资源与不重复产出

图鉴版保留原有商店价格、制作入口和旅行界面，但资源逻辑改为稳定的展示与试用状态：

```text
三叶草
→ 固定为 9999

食品、幸运物、花朵、花苗、种子、特产与常规材料
→ 各保有 99 个
...
相册页数已经扩展到最大可用容量，固定照片各保留一份；动态照片所需的六类材料照片也一并准备好，可以直接进入制作流程。花瓶中的可插花材、周年蛋糕材料和手工品材料保持 99 个，活动页面不显示结束日期，中秋、新年和周年庆可以随时查看和操作。

图鉴版仍允许青蛙制作家具、制作手工品并出现对应动作。区别在于：已经拥有的家具、祈愿物和印章不会重复写入收藏，也不会重复发放额外奖励。这样既能保留庭院的生活感，也不会破坏全图鉴状态。

### 2.一次性收集内容全图鉴

图鉴版进入游戏时已经补齐下列内容；它们仍会按照客户端原有分类显示，而不是单独做一套新的展示页：


* **旅行装备**
→ 食物、幸运物、工具、坐垫、杯子与旅行用具

* **家具与图纸**
→ 14 个正式主题、普通家具、全部不倒翁、堆肥箱、挂兜及相关设计图

* **故事与旅友**
→ 全部旅行邂逅故事、友情绘本页与收藏页

* **手工品**
→ 木质护符、祈愿物、印章、制作工具与可用材料

* **称号与百科**
→ 全部称号、物品百科、旅行百科、纪念品、特产、藏品与博物馆典藏

* **地图与旅行记录**
→ 全部地图区域、旅行笔记与可查看的目的地内容

* **伴蛙前行**
→ 30 个任务、所有阶段进度和全部奖励均标记为完成并领取

* **美好瞬间**
→ 30 个彩蛋

值得注意的是手工品部分：护符本体、祈愿物和印章都已纳入图鉴；三种护符碎片从零开始，仍可通过旅行获得并完成三拼。如果护符碎片也设成99且使用不消耗，那么就永远拼不完，也退不出拼护符的界面了。

### 3.照片、相册与动态照片

图鉴版的相册扩容了999页，并预先放入每一张非 `Normal` 类型的固定照片，包括旅行地点、食物、幸运物、活动和特殊事件照片。普通过路照属于随装备、旅伴与随机位置生成的照片，不作为固定样张预置，仍可以通过旅行继续产生。

动态照片的每一种来源照片都会预置六张，正好覆盖一次动态照片制作所需的材料数量；动态照片本体、相框入口和相册保存流程仍按原来的步骤运行。相册扩容道具保持最大容量，但当前可翻页数只取决于已经保存的真实照片，因此不会再出现“容量有一千页、后面全是空白”的页面。

### 4.活动与长期入口

普通版和图鉴版都将博物馆大冒险、扭蛋机、许愿池作为离线长期入口；它们不会因为原活动结束而消失。广告、分享类任务在两版中都保留按钮和奖励路径，离线环境下点击即可完成对应条件。
...
→ 中秋前后十天
→ 农历新年前后十五天
→ 周年庆 12 月 14 日至次年 1 月 12 日
→ 六一当天登录奖励与称号

图鉴版
→ 中秋贺卡、春节贺卡、周年庆蛋糕始终可进入
→ 不显示结束日期
→ 周年蛋糕所有层级同时可做
```

春节贺卡的首包免费、背景首次免费、贴纸组合、红箱与金箱奖励规则仍然存在；中秋贺卡的寄送、购买和奖励流程也未被删去。图鉴版只是解除日历窗口，让这些页面可以随时查看和操作。嘟嘟商人的普通商品、联动家具、幸运物、半价正丹纸和重复购买返还逻辑仍按各自物品规则处理。

### 5.庭院、制作与不重复奖励

图鉴版没有把青蛙变成静态陈列。青蛙仍会回家、外出、睡眠、阅读、进食、制作家具和制作手工品；花圃、花瓶、堆肥、挂兜、旅友到访、聚会、旅行返程和周末小插曲也仍能继续体验。
...
日历的红点不再仅仅因为节气任务全部完成就常亮。它会同时检查当天是否真的存在可领取的节气奖励；未来日期的奖励仍会留在日历中，但不会提前提示。幸运日奖励则继续按实际未领取状态显示红点。

相册扩容也重新回到它应有的职责：扩容只增加可存放照片的容量，而不制造没有照片的可翻页面。此前动态照片选择页和普通相册都会把容量当作实际页数，因此在已有照片页之后还能翻到空白或占位页。现在两种入口都只按真实照片数量分页：
...
周年蛋糕的小问答同时核对了客户端实际提交的答案字段。问答现在会正确识别所选项、发放奖励并推进下一步；普通版仍保留材料消耗、分阶段等待、制作、分享和领取奖励的顺序。

### 2.青蛙动作刷新与写字台入口

原版本青蛙动作刷新时长为固定2小时，现在设定为1-3小时随机。写字台原本没有对应的功能，现在点开写字台可以进入笔记界面。

### 3.嘟嘟商店、节日趣闻与新手旅行

嘟嘟的货架继续收紧到真正可购买的内容。实用锯子、毛刷、锤子、棒针、小刀等制作工具都是单次获得物品：已经拥有后不会再次进入当天货架。旧存档里遗留的同类货位也会在刷新时清掉；如果客户端保留了旧货位并继续点购，则保持原价返还，不会重复增加工具。

“故事”中的节日趣闻恢复为原存档中的两张明信片。它们作为过去与其他玩家互动留下的静态记录继续显示，不再由离线服务器虚构新的陌生玩家来信或改写这两张内容。
...
已有旅行历史的普通版与图鉴版不会被这条规则改写。

### 4.归家插花与木质护符碎片

青蛙旅行归家后，如果花瓶为空且仓库中有可插花材，会有35%的概率自己插上一朵花。它不会替换玩家已经插好的花，也不额外消耗花材；花朵仍按花瓶原有的新鲜、枯萎与消失周期推进。

木质护符的三种碎片不再按“缺哪种补哪种”的方式掉落。旅行触发碎片奖励后，三种碎片各占三分之一概率；即使某一种已经很多，仍有可能继续获得它。图鉴版保留护符本体99个、使用不消耗的设定，碎片则仍从旅行中随机得到，能够正常完成三拼。

### 5.周年蛋糕完成态与图鉴版抽奖券

周年蛋糕点燃蜡烛并领取最终奖励后，服务端会把蛋糕推进到完整的第六阶段。顶部五个进度图标因此都会显示为已点亮，最后的蜡烛不再停留在灰色。旧的已完成蛋糕记录也会在加载时自动校正到这一状态。

图鉴版的抽奖券补齐为与三叶草一致的固定资源：数量始终显示999，商店抽奖不扣减，白球返券或其他获得途径也不会继续增加。普通版仍使用实际抽奖券数量与消耗规则。

### 6.dry 过路照的滴水叶片坐标

过路照中 `dry`、`dry1`、`dry2`、`dry3` 共用的滴水叶片前景曾回退到左上角 `(0,0)`，导致叶片和水滴压住青蛙。该前景资源宽242，而照片画布宽500；现在按右侧贴边的位置放到 `(258,0)`：
